...
class generator():
    ...
    def pv_accumulate(self, device_type, device_no, sleep_time):
        scope = device_no + '_' + config_get("device_epc", "pv_cumulative")
    ...
    def battery_accumulate(self, device_type, device_no, scope, sleep_time):
        epc = config_get("device_epc", scope)
        scope = device_no + '_' + config_get("device_epc", scope)
        verify_init_value_isexist(device_type, scope)
        str_url = config_get("api_info", "battery_url").format(device_no, epc)
    ...
    def fc_accumulate(self, device_type, device_no, sleep_time):
        scope = device_no + '_' + config_get("device_epc", "fc_cumulative")
        verify_init_value_isexist(device_type, scope)  
        str_url = config_get("api_info", "fc_url").format(device_no, config_get("device_epc", "fc_cumulative"))
        str_device_scope = device_no + '_' + config_get("device_epc", "fc_cumulative")
        int_start_data = int(config_get(device_type, str_device_scope))
        int_data = int_start_data
        while(True):
            str_data = str(int_data)
    ...
    def pdb_accumulate(self, device_type, device_no, scope, sleep_time):
        epc = config_get("device_epc", scope)
        scope = device_no + '_' + config_get("device_epc", scope)
        verify_init_value_isexist(device_type, scope)
        str_url = config_get("api_info", "pdb_url").format(device_no, epc)
        str_device_scope = device_no + '_' + epc
        int_start_data = int(config_get(device_type, str_device_scope))
        int_data = int_start_data
        while(True):
